...
def create_backbone_model(input_size=(512, 512, 3), block_size=7, keep_prob=0.9, 
...
def create_sa_unet_model(input_size=(512, 512, 3), block_size=7, keep_prob=0.9, 
                        start_neurons=16, use_output_activation=True):
...
if __name__ == "__main__":
    ...
    # Test single-channel input data 1x1x1500x1500
    x = torch.randn(1, 1, 1500, 1500).to(device)
    
    # Test model with activation function
    model_with_activation = create_sa_unet_model_for_single_channel(use_output_activation=True).to(device)
    output_with_activation = model_with_activation(x)
    
    # Test model without activation function
    model_without_activation = create_sa_unet_model_for_single_channel(use_output_activation=False).to(device)
    output_without_activation = model_without_activation(x)
    
    print(f"Input shape: {x.shape}")
    print(f"Output shape (with activation): {output_with_activation.shape}")
    print(f"Output value range (with activation): [{output_with_activation.min().item():.4f}, {output_with_activation.max().item():.4f}]")
    print(f"Output shape (without activation): {output_without_activation.shape}")
    print(f"Output value range (without activation): [{output_without_activation.min().item():.4f}, {output_without_activation.max().item():.4f}]")
    print(f"Model parameters: {sum(p.numel() for p in model_with_activation.parameters())}")
