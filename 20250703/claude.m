...
% New parameters for multiple PL stars and broken lines
CONFIG.NumPLStars = randi([1, 3]);  % Random number of PL stars (1-3)
...
for i = 1:length(matFiles)
    ...
    CONFIG.NumPLStars = randi([1, 3]);   % Random number of PL stars for each file
    fprintf("Generating %d PL star(s) for this wafer\n", CONFIG.NumPLStars);
...
function img = generatePLStarWithBreaks(width, height, centerX, centerY, ellipseMinor, ellipseMajor, lineWidth, waferImg, CONFIG)
    ...
    % Draw the lines on the image with potential breaks
    for i = 1:length(angles)
...
function img = drawLineWithBreaks(img, waferImg, x1, y1, x2, y2, width, CONFIG)
    ...
    % Draw only the non-break segments
    for i = 1:lineLength
        if ~drawMask(i)
